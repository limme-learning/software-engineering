...
Enterprise estates are consolidating faster than they are being rewritten: insurers keep buying books of business, banks keep acquiring fintechs, and every acquisition arrives with a rating engine, a policy admin system or a ledger that will outlive the integration project. That makes the adapter boundary a permanent structure, not a migration artefact — and the pressure on it is increasing, because operational-resilience and third-party-risk regimes now expect firms to demonstrate they can substitute a critical provider, which is only cheap if the provider sits behind one port. Meanwhile the industry's move to standard schemas — ISO 20022 in payments, common rating and claims models in insurance — does not remove adapters, it just makes their target interface easier to agree on. AI-assisted development shifts the economics in a specific way: generating an adapter from a WSDL or OpenAPI document is now close to free, so the scarce judgement is deciding *what your side of the boundary should look like* and refusing to let the generated shape become your domain model. Teams that keep that boundary explicit will keep absorbing vendor churn in a single file; teams that don't will keep paying for it in customer-remediation letters.
