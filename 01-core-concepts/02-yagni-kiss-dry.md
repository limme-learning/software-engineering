...
The economics behind YAGNI are shifting in its favour. As the cost of writing code falls, the cost of *understanding* code becomes the dominant constraint on delivery — and speculative abstraction spends exactly that resource. A plain, explicit calculation that a domain expert can read is now worth more relative to a clever generalisation than it was when typing was the bottleneck. This applies with particular force to machine-assisted development: generation tools reproduce and extend whatever patterns they find, so an unnecessary abstraction does not stay one file — it becomes the template for the next twenty. Meanwhile, regulated domains keep moving toward requirements that business logic be inspectable and explainable, which favours code that states its rules directly over code that assembles them at runtime from a registry. Simplicity has always been an engineering virtue; it is increasingly a compliance one.
