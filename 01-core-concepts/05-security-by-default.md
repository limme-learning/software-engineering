...
Broken access control has stayed at the top of the OWASP Top 10 not because the technique is hard but because it is *specific to your domain* — no framework, gateway, or WAF can infer that a purchase order belongs to an organisation, so this is permanently application-level work. Three pressures increase the stakes. Multi-tenancy is now the default architecture for enterprise software, which means every missed scope check is a cross-customer exposure rather than a single-user one. Regulatory regimes are attaching real consequences to that class of exposure, with disclosure obligations measured in days. And an increasing share of application code is generated from prompts and specifications, where the tenant scope is exactly the kind of implicit business constraint that is easy to omit and impossible to detect by reading the code in isolation — a `findById` looks perfectly correct. The durable defences are therefore structural rather than vigilant: deny by default in configuration, repositories that expose no unscoped query, row-level security beneath them, and cross-tenant tests that fail the build. Build the system so the vulnerable version does not compile or does not pass CI, because attention alone does not scale.
