...
Data outlives every application that touches it. The service enforcing today's rule in Java will be rewritten, wrapped, or replaced by a new platform within a decade; the table will still be there, and so will the rows inserted by whatever else reached it in the meantime. That asymmetry is the whole argument for putting invariants where every writer must pass through them. Two trends sharpen it. Enterprise data estates are becoming more polyglot, not less — analytics pipelines, reverse-ETL tools, low-code platforms, and AI agents all acquire write paths to operational tables, and none of them will run your service's validation. And regulatory attention is shifting toward data quality and fairness outcomes rather than process alone: an auditor asking why two identical claims settled differently is asking a data-integrity question, and "our application layer prevents that" is not an answer when the bad rows are visibly present. Declarative constraints are the cheapest durable evidence that a rule was actually enforced.
