...
The economics of debt are shifting in a way that makes measurement more important, not less. Code is becoming cheaper to produce, which lowers the cost of the *repayment* side of the ledger — extracting a module, writing the missing tests, generating characterisation fixtures are all getting faster. But the interest side is dominated by things that are not getting cheaper: understanding what the existing code does, being confident a change is safe, and getting it through review and validation in a regulated estate. So the gap widens between codebases that are comprehensible and codebases that are not, and it widens fastest exactly where teams lean hardest on generation — an assistant working in a coherent, tested module accelerates delivery, while the same assistant in a 9-year-old untested one produces plausible changes nobody can verify. At the same time, regulatory expectations around change delivery are tightening, with mandated changes arriving on statutory clocks that do not negotiate. That converts technical debt from an engineering-quality concern into a delivery-capability risk, which belongs on a risk register with a number attached — and a number is something the business will act on.
