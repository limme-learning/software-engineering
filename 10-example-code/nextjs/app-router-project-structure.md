...
Filesystem routing is now the settled convention across the React ecosystem — App Router, Nuxt, SvelteKit, TanStack Router all converge on it — so the shape of `app/` is not the thing that will churn. What churns is where the logic sits, and that is exactly what this structure pins down: routes are adapters, features are the application, `shared/` is the only sanctioned crossing point. That separation survived the move from Pages Router to App Router, and it will survive whatever the next rendering model is, because a feature module that exposes `ClaimDetail` and `claimSchema` does not care how it is mounted. The pressure making this more valuable, not less, is organisational. Enterprise frontends keep accumulating squads faster than they accumulate routes, and AI-assisted development amplifies whatever boundaries already exist — an agent asked to change claims logic in a well-bounded repo touches one folder, and in a flat repo touches forty files. Explicit `cache` and `revalidate` on every fetch matters for the same reason: as caching defaults shift between framework versions, code that states its intent keeps behaving the way the audit trail says it does.
