...
The split between "my own UI" and "a consumer I do not control" is an architectural fact, not a framework detail, so it outlives whatever the RPC mechanism is called next year. Server Actions replaced hand-rolled `fetch('/api/...')` for forms and will keep absorbing that role as React's server-function model stabilises across frameworks; the HTTP endpoint keeps its place precisely because a treasury job in Python and a partner in Java need a URL, a status code, and a documented error body. RFC 9457 problem details are the durable investment there — it is now the default error format in API gateways and in OpenAPI tooling, and a machine caller that can branch on `type` instead of parsing English prose is the difference between a self-service integration and a support queue. Regulated environments push the rest: PSD2-style open banking, DORA, and internal audit all want per-principal rate limits, idempotency on money movement, and an auth decision traceable to a verified token rather than a header. Write validation once as a Zod schema and both entry points inherit it, which is what stops the two paths drifting as the product grows.
