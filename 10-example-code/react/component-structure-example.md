...
React's own direction reinforces this structure rather than replacing it. The server/client split makes the boundary between "fetches data" and "handles interaction" a compile-time fact instead of a convention, which rewards exactly the container/presentational separation shown here — the further down the tree `'use client'` sits, the less JavaScript ships. Compound components rely on nothing newer than context and closures, so they survive framework churn: the same pattern written for React 16 works unchanged in React 19. Meanwhile the pressure that creates twenty-three-prop components is structural, not technological — in regulated environments, legal, audit, fraud and operations will always want the same screen with different columns and different permissions, and they will always ask one at a time. Code generation raises the stakes: an assistant asked to "add a prop for the fraud team" will happily do it, and the only defence is a component whose shape makes the wrong change awkward and the right change obvious. The folder with one public surface is what lets a team still delete something in year four.
