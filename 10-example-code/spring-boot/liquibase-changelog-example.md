...
Schema evolution is the one part of a system you cannot roll back by redeploying the previous container image, and that asymmetry is permanent — it does not care which framework, language, or cloud you are on. The expand/contract sequence is durable for the same reason: as long as deployments are rolling and availability targets are measured in nines, there will be a window where two application versions must read one schema, and the only way through it is additive change followed by a guarded cleanup. What is intensifying is the evidence requirement. Auditors and regulators increasingly ask not whether you have migrations but whether you can produce the forward SQL, the reversal SQL, the approval, and proof that what ran matches what was approved — which is exactly the set of artefacts `update-sql`, `rollback-sql`, and the `databasechangelog` checksums produce as a by-product. Liquibase may not be the tool you use in 2030, but whatever replaces it will need identified, checksummed, reversible changesets with preconditions, because that list is not a product feature; it is the shape of the problem.
