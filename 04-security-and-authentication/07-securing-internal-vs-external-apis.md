...
Every structural trend in enterprise architecture makes the perimeter weaker and workload identity more valuable. Estates are already spread across on-premise, multiple clouds and SaaS, so there is no single network edge left to defend; hybrid working dissolved the corporate network as a trust signal; and supply-chain compromise means the hostile code may arrive inside your own build rather than through your front door. Regulation has caught up — DORA and comparable operational-resilience regimes expect firms to demonstrate segmentation, least privilege and provable access control between internal components, not merely at the boundary. The newest pressure is autonomous agents calling internal APIs: an agent is exactly the caller you must not trust by location, and the only sane way to constrain it is a narrowly scoped, short-lived, revocable credential attributable to whoever it acts for — the same machinery described here. The tooling will keep shifting; ambient mesh and sidecar-less data planes are lowering the cost of mTLS, and SPIFFE is becoming the common vocabulary across platforms. The principle underneath does not move: identify the caller, authorise the operation, scope the data, record the actor. On both sides of the firewall.
