...
Consolidating identity is one of the few architectural moves in a bank that pays back in security, operations, and audit simultaneously, and the direction of regulation keeps pushing it: strong customer authentication, periodic access review, and operational-resilience regimes such as DORA all assume a single place where identity is asserted and a single place where access can be proved and withdrawn. That same consolidation is what makes the IdP a tier-0 dependency, and the engineering discipline it demands — clustered deployment, an HA database, cached JWKS, tested failover, a documented degraded mode — is not going to get easier as more of the estate depends on it. Token exchange is the piece to invest in early. As service estates grow and, increasingly, as autonomous agents start calling internal APIs on a user's behalf, the question "what is the narrowest credential this hop needs, and whose authority is it acting under?" becomes the central access-control question, and RFC 8693 with `may_act` is the standardised answer rather than a vendor feature. The specifics of Keycloak's operator or cache stack will change across the decade; one session, many audiences, narrow tokens per hop, and server-side logout will not.
