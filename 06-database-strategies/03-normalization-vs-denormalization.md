...
The forces on both sides of this trade-off are strengthening at once, which is why the judgment stays relevant rather than being automated away. On the write side, regulatory expectations around data lineage and reproducibility are rising — a supervisor asking how a reported figure was derived is asking for a path back to source records, and a denormalised column maintained by forgotten application code cannot provide one. On the read side, demand for low-latency analytical access keeps growing, now including AI-driven tooling that will happily issue nine-way aggregate joins against whatever database it is pointed at, at machine frequency. The tooling is improving in a way that helps: cheap columnar replicas, logical-replication-fed read stores, and change-data-capture pipelines make the "separate the read path" option far less exotic than it was five years ago, and the honest default in 2026 is that heavy reporting should not touch the OLTP primary at all. What does not change is the underlying asymmetry — a slow report is an inconvenience, a wrong number in a regulatory return is a finding — and that asymmetry is why the duplicate should always be derived, refreshable, and reconciled, never hand-maintained.
