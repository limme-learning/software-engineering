...
Retention requirements are ratcheting in one direction: financial and insurance regulators keep extending how long records must remain queryable, and privacy regimes simultaneously require provable deletion at the end of that period. A table you can neither shrink nor prove you have purged is a compliance liability, and `DETACH PARTITION` plus an exported dump is the cheapest credible evidence that a retention rule was executed on a specific date. The mechanics keep improving rather than being replaced — PostgreSQL has added concurrent detach, better runtime pruning, and partitionwise joins and aggregates across recent majors, and the trajectory in 16/17 is toward the planner handling more partitioned shapes natively. Managed platforms and extensions such as pg_partman or Citus automate the bookkeeping, but none of them can choose your partition key, and that choice is the decision that determines whether every future query prunes or fans out. Getting it right early is cheap; changing it on a two-terabyte table in a regulated system is a project.
