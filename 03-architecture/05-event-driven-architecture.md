...
Event-driven integration is becoming the default shape of enterprise systems for reasons that have nothing to do with fashion: instant payment schemes, ISO 20022 messaging, open-banking notification requirements, and real-time fraud scoring all force asynchronous, fact-carrying flows on institutions whether or not they wanted them. As that spreads, the outbox pattern and idempotent consumers are moving from "senior engineer knowledge" to table stakes — and the tooling has followed, with Debezium, Spring Modulith's event publication registry, and managed broker offerings taking over mechanics that used to be hand-rolled. What tooling will not do for you is the modelling: whether a message is a fact or a request, which entity's order must be preserved, and what compensating action exists when step four fails after step three committed money. Those decisions are where incidents come from, they are the ones auditors ask about, and they will still be yours in 2030.
