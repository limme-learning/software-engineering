...
The dependency rule survives because it answers a question that is getting louder, not quieter: can you run your business rules without your infrastructure? Insurance and banking supervisors increasingly expect model explainability and reproducibility — for a given decision on a given date, which rule version applied and can you re-execute it — and that is only cheap when the rule is a pure function over versioned inputs. The same property is what makes rules reusable across a Java backend, a browser quote widget, and a batch replay harness, and what makes them testable at the volume that regulatory evidence now requires. There is a second, newer reason: when rules are pure functions with explicit inputs, they are also the safe surface to expose to automated and agentic callers, because the boundary validates itself and nothing downstream depends on who called. Where clean architecture will keep losing ground is the place it never belonged — CRUD, reference data, and reporting, where frameworks have grown genuinely good defaults and extra rings buy nothing. Knowing which half of your codebase you are in is the durable skill.
