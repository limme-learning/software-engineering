...
The economics behind these choices have shifted in a way that makes deliberate selection more valuable, not less. Cloud cost scrutiny turned per-service overhead into a defended budget line; operational-resilience regulation counts the components in a critical business service, so every extra deployable is another dependency to map, test and prove survivable; and platform tooling has finally made the middle ground credible, with build-time module verification and virtual threads removing two of the standard arguments for splitting early. AI-assisted development pushes the same direction: a model reasons well about a large codebase with explicit, enforced boundaries and badly about thirty repositories whose contracts live in a wiki, which raises the value of boundaries that are visible in source over boundaries that are visible only in network traffic. What none of this changes is the judgment. Tools will generate a hexagonal skeleton or a service scaffold on request, and that makes it cheaper than ever to build the wrong structure quickly. The durable skill is the one this article is about: naming the property you are buying, knowing what the purchase costs, checking whether you can undo it, and writing down why — so that in 2030 someone can tell whether the reason still holds.
