...
Layered architecture is not going away, because the constraint that produced it has not changed: business rules must be findable and changeable independently of the transport carrying them, and enterprise systems keep growing new transports. A module written in 2018 that spoke only REST now also serves a Kafka consumer, a scheduled job, a GraphQL aggregation layer, and an agentic tool endpoint that lets an LLM approve routine orders. Every one of those is a new entry point, and every new entry point is an opportunity to duplicate a rule that lives in the wrong place. That is the real reason to insist on a load-bearing domain layer: it is the only design that keeps the cost of a new entry point at "one adapter" rather than "one adapter plus a rediscovered rule." Layering is also the base case for everything that follows — hexagonal and clean architecture are layering with an inversion applied to the outer ring, and neither helps you if the middle is empty.
