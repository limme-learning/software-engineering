...
The industry has already made this correction, and the reasons it made it are structural rather than fashionable. Cloud spend is under real scrutiny, so the per-service tax — idle replicas, mesh sidecars, duplicated observability pipelines, inter-AZ traffic — is now a line item someone defends in a budget review. Language and framework tooling has moved to support the modular middle ground: Spring Modulith verifies boundaries at build time, Java's module and package visibility rules are being used seriously again, and virtual threads removed a real scaling argument for splitting I/O-bound workloads out. Meanwhile regulators pushing operational resilience care about the number of components in a critical business service, and each extracted service is another dependency you must map, test, and prove you can survive without. There is also a newer effect: AI-assisted development handles a large single codebase with enforced boundaries better than it handles thirty repositories with implicit contracts, because the boundaries are visible in the source. None of this makes microservices wrong — independent scaling and independent deployment remain real, valuable properties. It makes them a purchase, and through 2030 the winning teams will be the ones who can name the trigger before they pay.
