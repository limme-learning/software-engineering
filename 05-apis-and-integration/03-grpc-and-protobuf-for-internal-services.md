...
The durable value here is not the wire format — it is that the contract is a compiled artefact rather than a document, and that property only becomes more valuable as estates grow. Protobuf has been wire-stable for over a decade and gRPC is a graduated CNCF project embedded in Kubernetes, Envoy, Istio, and effectively every service mesh, so the tooling risk is close to zero. Three forces keep pushing internal traffic toward it: service counts keep rising, so per-hop serialisation cost and cross-service contract drift both compound; polyglot estates are now normal, and a shared `.proto` is the cheapest way to keep a Java pricing engine and a Python scoring model honest with each other; and AI-adjacent workloads — model inference, document extraction, streaming scoring — are natively streaming and latency-sensitive, which is precisely gRPC's shape. Meanwhile HTTP/3 and QUIC are arriving underneath gRPC rather than replacing it, so the investment carries forward. The pattern to bet on is the boundary split: protobuf contracts with `buf breaking` in CI inside the estate, REST with an OpenAPI document at the edge. That division has survived every framework fashion since 2016 and there is nothing on the horizon that dissolves it.
