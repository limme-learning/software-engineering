...
None of the three styles is going to displace the others, and the reason is that their differences are structural rather than fashionable. REST wins the partner edge because regulators and procurement departments require a machine-readable contract that any stack can consume with an HTTP client, and open-banking and open-insurance schemes have decade-long lifecycles specified exactly that way. gRPC wins inside the estate because a compiled contract is the only mechanism that turns a field rename into a build failure, and service counts keep rising. GraphQL keeps the aggregation problem because enterprises accumulate systems instead of replacing them, so "one screen, six backends, none of them ours" gets more common every year. What will change is the surrounding rigour: contract testing and spec-diffing in CI are becoming table stakes at every boundary, field-level access logging is turning from a nice-to-have into an explicit control, and automated consumers — agentic clients reading your OpenAPI document and calling endpoints unattended — are about to put real load on the partner edge with none of a human integrator's forgiveness. The decision that ages well is therefore not a style, it is the boundary map: write down where each boundary is, which style crosses it, what the source of truth for the domain is, and which CI gate protects each contract. Teams with that document adapt when a fourth style appears. Teams with a one-line standard will hold another governance board meeting.
