...
The regulatory direction is unambiguous and it is toward evidence rather than documentation. DORA has made testing, resilience validation and third-party assurance explicit operational obligations for EU financial entities rather than good practice; the EU AI Act imposes documented testing and record-keeping duties on high-risk decisioning, which includes a great deal of credit and underwriting logic; and data-protection enforcement has settled the question of masked production data decisively enough that "we removed the names" is no longer a position anyone defends twice. At the same time, supply-chain requirements — SBOMs, provenance attestations, signed artifacts — are converging on exactly the same primitive this article is built on: a machine-readable bundle of facts about a build, immutable, keyed by artifact digest, retained. That convergence is good news, because it means the evidence pipeline you build for an auditor is the same one you build for provenance, and neither is extra work once the plumbing exists. The force that makes it more urgent is generated code: when a large share of a change is machine-authored, the interesting question moves from "did a person write this carefully" to "what independent, retained, executable evidence exists that it behaves correctly and that the controls around it hold". Teams that can answer with a URL and a digest will pass examinations at the speed of a query. Teams that answer with a screenshot of a green pipeline will keep slipping releases by a quarter.
