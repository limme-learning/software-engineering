...
The economics here have already settled and will not reverse: container startup keeps getting cheaper, CI runners keep getting more capable, and the marginal cost of testing against the real engine is now measured in seconds. That removes the last defensible reason anyone had for in-memory substitutes. The pressure that *is* increasing comes from generated code — AI assistants produce plausible repository methods and plausible tests with equal fluency, and a mock-heavy test written alongside generated code will happily assert the generated mistake back at you. Real infrastructure is the check that does not share the generator's assumptions: PostgreSQL either accepts the query or it does not. The layer that will matter most through 2030 is exactly this one, because it is the cheapest place to get an independent verdict on whether code works, and independence is the property that generated tests lack.
