...
Tokens are durable because they are a naming and indirection problem, and those outlive frameworks. The specific plumbing has already changed twice — SASS variables, then JS theme objects, now native CSS custom properties with `@theme` and OKLCH — and the three-tier structure survived each migration unchanged, because what it encodes is a decision hierarchy, not a syntax. Two forces are increasing the pressure. Accessibility regulation with real procurement consequences (the European Accessibility Act, Section 508 refreshes) makes contrast a pass/fail property of a *named token* that can be tested once, rather than of 1,847 literals that must each be audited. And AI-assisted code generation raises the cost of ambiguity sharply: a model given `--color-surface-danger` in scope and a lint rule that rejects hex will produce consistent output, while a model given a codebase full of near-identical reds will faithfully reproduce the inconsistency at machine speed. The systems that stay coherent through 2030 will be the ones where the correct value is the easiest value to reach and the wrong one cannot merge.
