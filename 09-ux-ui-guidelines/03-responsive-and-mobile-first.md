...
Device diversity is widening, not converging: foldables with two distinct postures, tablets used as primary work machines, ultrawide desktops, and vehicle-mounted terminals all coexist in the same enterprise fleet, so no single breakpoint set will ever be sufficient — which is precisely why the durable technique is per-view intent plus container-relative components rather than a fixed breakpoint table. The CSS platform has moved decisively in that direction and the primitives are now stable: container queries, `clamp()`, logical properties, and `:has()` all have universal support and all express layout as a function of available space rather than of a device guess. At the same time reflow and zoom have hardened from good practice into audited legal requirements under the EAA and EN 301 549, which means the 400% check is now evidence in a procurement document rather than a nicety. The habit that pays off through 2030 is unglamorous: write down which form factor each view is for, build components that ask their container rather than the window, and verify zoom before design sign-off.
