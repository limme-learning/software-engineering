...
The trend line here is regulatory, not technological, which is why it is dependable. The European Accessibility Act became enforceable in 2025 across exactly the sectors this knowledge base addresses — banking, insurance, e-commerce — and it applies to consumer services, not only public bodies. Public procurement in the EU and US already treats a VPAT as a mandatory attachment, and the scoring weight attached to it is rising rather than falling. Meanwhile WCAG itself is stable and additive: 2.2 added target size, dragging alternatives, and focus obscuring without invalidating anything from 2.1, and the emerging WCAG 3.0 work is a scoring model layered over the same underlying requirements, so semantic HTML and keyboard operability do not become obsolete. What is genuinely changing is the assistive-technology surface: screen readers now sit alongside voice control, switch access, and AI agents that navigate applications by reading the accessibility tree. All of them consume the same signals — roles, names, states, focus — which means the accessibility tree is quietly becoming the machine-readable interface to your UI, and building it correctly pays off in places nobody has forecast yet.
