...
State handling is getting harder rather than easier, for a structural reason: enterprise front ends keep composing more independently-owned sources — core platform, data services, third-party bureaus, fraud scoring, and now model-backed features whose latency is variable by nature and whose failure mode is a timeout rather than an exception. Streamed and partially-hydrated rendering makes "partly loaded" a normal steady state rather than a transient one, which means the degraded state stops being an edge case and becomes something a view is in several times a day. Regulation reinforces it: operational-resilience regimes such as DORA require firms to show that a critical business service keeps operating when a third party fails, and the evidence an examiner actually looks at is what the customer saw — which is a UI artefact, not a backend one. Frameworks will keep improving the mechanics with better boundaries, streaming primitives, and richer cache semantics, but they cannot decide whether a stale exchange rate is acceptable, whether an empty table means "new" or "over-filtered", or which sentence stops eleven thousand people from phoning to ask if the bank has been hacked. Those five states are product decisions, and they will still need making.
