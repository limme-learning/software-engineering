...
The durable idea here is not Tailwind — it is that visual decisions must live in a single enforced token layer, and that the enforcement must run in CI rather than in a wiki. That holds whether the utilities come from Tailwind, from a native CSS-modules design system, or from whatever replaces both, because the pressures pointing at it are getting stronger, not weaker: WCAG 2.2 is now the baseline that European accessibility legislation references, so contrast and focus visibility are compliance artefacts an auditor will sample, and a codebase where colour is defined 4,000 times cannot answer an auditor at all. Tailwind v4's shift to CSS custom properties as the token substrate matters precisely because it aligns with the platform rather than a build tool — those variables are inspectable in devtools, themeable at runtime, and consumable by non-Tailwind code, which means the token layer outlives any decision to change framework. The enforcement habit is the part to carry forward: tokens in one file, no arbitrary values in components, and a pipeline that says no.
