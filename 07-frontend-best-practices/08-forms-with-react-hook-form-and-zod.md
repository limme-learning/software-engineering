...
The principle here predates React and will outlive it: a rule enforced in two places is a rule that will eventually disagree with itself, and the enterprise cost of that disagreement is not a bug ticket but a control failure — an account opened on unverifiable data, a claim accepted outside policy terms, a payment instruction that passed the screen and failed the ledger. What has actually changed is that TypeScript now lets the rule and the type be the same artefact, so a schema is simultaneously validation, documentation, and the compiler's opinion about your data — which is why the pattern has propagated well past Zod, into Valibot, ArkType, and the Standard Schema interface that lets a resolver accept any of them. That interoperability matters more than the library choice: it means the shared-contracts package survives a validation-library migration, which is the durability you actually want from an architectural decision. Server Components and server actions push the same way, by making it natural for the validation to sit next to the mutation rather than in a client bundle. So the thing to carry into 2030 is not `zodResolver`; it is the refusal to write a rule twice, and the discipline of treating the client check as a courtesy and the server check as the control.
