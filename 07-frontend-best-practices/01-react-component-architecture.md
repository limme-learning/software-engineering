...
Composition is not a React idea; it is the only structural tool React exposes, and it has survived every major version because it is a property of the model rather than an API. The server/client split is newer but is converging, not diverging: Vue, Svelte, and Angular have all landed some form of server-rendered-by-default with islands of interactivity, which means the mental discipline of asking "does this need a browser?" per component transfers across frameworks rather than locking you into one. The React Compiler will keep absorbing manual memoization, and that is fine — it removes busywork, not judgment. What it will never do is decide where your error boundaries go, which regions of a page are independently valuable, or what belongs on the wrong side of a serialization boundary. Those are design decisions with measurable consequences in bundle size and perceived reliability, and in regulated enterprise UIs, where a screen must stay usable when one panel's upstream service is down, they are the difference between a degraded page and a blank one.
