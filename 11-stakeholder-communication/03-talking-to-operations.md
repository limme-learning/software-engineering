...
The trend that makes this permanent is the widening gap between how fast code ships and how fast an organisation can absorb a change. Deployment automation, AI-assisted implementation, and platform tooling keep compressing build and release time, while the human systems around a change — on-call rotations, contact-centre scripts, change records, regulatory notification duties — do not compress at all. Every increase in deployment frequency multiplies the number of handovers, so the per-change cost of a bad handover becomes the dominant term. Regulation is pushing in the same direction: operational-resilience regimes like DORA and the FCA's operational-resilience rules require firms to demonstrate not just that a system recovered, but that they knew what changed, could reverse it, and communicated the customer impact within defined timeframes — which is precisely the runbook, the rollback, and the support brief, formalised. Automation will keep improving the plumbing: alerts will link runbooks automatically, rollbacks will be one-click, incident summaries will be drafted for you. What it will not do is decide what a customer will phone to complain about, or which team should receive that call. The engineer who writes those two paragraphs before the release is the one whose changes stop turning four-minute events into two-hour incidents.
