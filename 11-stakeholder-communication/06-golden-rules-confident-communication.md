...
These habits don't decay the way specific tools or frameworks do, because they aren't about technology — they're about what any reader, in any decade, needs from a message in order to act on it without a follow-up conversation. If anything, the value is rising: written asynchronous communication keeps replacing meetings as the default coordination mechanism, so the cost of an unclear update is no longer "someone asks a clarifying question in the room" but "a decision stalls for a day while people are in different time zones." And as AI tools make it trivially easy to produce fluent, well-punctuated prose, fluency stops being the differentiator it once was — the sentence that actually earns trust is the one with a stated conclusion, a real number, and an owned uncertainty with a date attached, and no amount of polish substitutes for having those three things.
